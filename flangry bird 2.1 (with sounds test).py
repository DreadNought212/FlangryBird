...
clarification = pygame.Surface((WIN_WIDTH, WIN_HEIGHT))
clarification.fill((255, 255, 255))
...
def game_over(pipes_list):
    ...
    while True:
        ...
        if clarification_opacity != 0:
            clarification_opacity -= 25
        ...
        blackout.set_alpha(blackout_opacity)
        window.blit(clarification, (0, 0))
        clarification.set_alpha(clarification_opacity)
...
def main():
    ...
    while run:
        ...
        for el_pipe in pipes:
            if el_pipe.pipe_up_rect1.colliderect(Bird(y_bird, 150, rotation_degree).bird_rect1) == True or el_pipe.pipe_down_rect1.colliderect(Bird(y_bird, 150, rotation_degree).bird_rect1) == True or y_bird >= 410 or y_bird <= 0:
